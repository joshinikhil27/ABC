...
import org.openecomp.dcae.restapi.endpoints.GenericAdaptor;
...
public class EventPublisher {
	...
	private static String vesUiUrl=RestfulCollectorServlet.getUiPath();
	...
	public void sendEvent(JSONObject event) {
		...
		if (event.has("VESuniqueId"))
		{
			...
			/*GenericAdaptor.getUuidToEventMap().forEach((k,v) ->
			{
				if(uuid.equals(k))
				{
			...
			try
			{
				for(Entry<UUID, List<String>> entry : GenericAdaptor.getUuidToEventMap().entrySet())
				{
