...
public enum EVEL_ERR_CODES {
  ...
  EVEL_MAX_ERROR_CODES            /** Maximum number of valid error codes.   */
      ...
      for(;;){
    	  if( (datatosend = (String) ringb.take()) != null )
    	  {
			  ...
			  try {
                ...
                default:
                    logger.error(url+" **unknown response code**:"+respCode);
                    break; // abort
                }
                ...
                if(es!=null){
                	System.out.println("safd"+es.toString());
    	  ...
    	  {
    		  System.out.println(url + "Waiting for events");
    		 /* try {
				Thread.sleep(5);
    	  ...
    	  System.out.println("Waiting infinitely");
      }//end for
	...
	public static boolean evel_post_event(EvelHeader obj )
    {
    	...
    	logger.info("Event : "+data+" Sent Successfully to VES Collector");
    	return ret;
    }
